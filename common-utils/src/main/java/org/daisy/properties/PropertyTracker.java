...
public class PropertyTracker {
	...
	@Subscribe
	public void handlePropery(Property prop){
	...
	@Subscribe
	public void handleProperyDeletionEvent(PropertyDeletionEvent prop){
		String propName=prop.getProperty().getPropertyName();
		String bundle=prop.getProperty().getBundleName();

		if (this.propertiesIndex.containsKey(propName)){
			if (this.propertiesIndex.get(propertiesIndex).containsKey(bundle)) {
				...
				this.properties.remove(prop.getProperty());	
			}else{
