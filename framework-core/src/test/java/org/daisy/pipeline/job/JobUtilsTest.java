...
public class JobUtilsTest {
        ...
        public void withoutValidationPort(){
                ...
                ResultSet results=new ResultSet.Builder().build();
        ...
        public void validationPortOk(){
                //should return valid when no validation port is present
                ResultSet results=new ResultSet.Builder().addResult("validation-status",new JobResult.Builder().withPath(ok.toURI()).build()).build();
                boolean valid=JobUtils.checkValidPort(results);
        ...
        public void validationPortError(){
                //should return valid when no validation port is present
                ResultSet results=new ResultSet.Builder().addResult("validation-status",new JobResult.Builder().withPath(err.toURI()).build()).build();
                boolean valid=JobUtils.checkValidPort(results);
        ...
        public void invalidValidationXml(){
               //should return valid when no validation port is present
                ResultSet results=new ResultSet.Builder().addResult("validation-status",new JobResult.Builder().withPath(invalid.toURI()).build()).build();
                JobUtils.checkValidPort(results);
        }
        @Test
        public void multipleValidationOk(){
                
                ResultSet results=new ResultSet.Builder()
