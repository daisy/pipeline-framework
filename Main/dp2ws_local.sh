...
$JAVA -Dorg.daisy.pipeline.local=true -Dorg.daisy.pipeline.ws.authentication=false  -Dorg.daisy.pipeline.home="$DP2_HOME" $DS_MODE -Dorg.daisy.pipeline.iobase="$TMP_IO_DIR" -Dorg.daisy.pipeline.logdir="$DP_LOG_DIR" -Dlogback.configurationFile="$LOGBACK_CONF" -Dfelix.config.properties="$FELIX_CONF" -Dorg.daisy.pipeline.xproc.configuration="$XPROC_CONF" -jar $DP2_HOME/fbin/felix.jar -b $DP2_HOME/bundle
