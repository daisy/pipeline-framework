...
public class StaxXProcScriptParser implements XProcScriptParser {
	...
	private XMLEvent readNext(XMLEventReader reader) throws XMLStreamException {
	...
	public void parseStep(final XMLEventReader reader,
			XProcPipelineInfo pipelineInfo) throws XMLStreamException {

		scriptBuilder.withPipelineInfo(pipelineInfo);
		...
		while (reader.hasNext()) {
			...
			if (event.isStartElement()
				...
				} else if (mAncestors.get(mAncestors.size()-2).asStartElement().getName()
						.equals(Elements.P_INPUT)
						|| mAncestors.get(mAncestors.size()-2).asStartElement().getName()
								.equals(Elements.P_OUTPUT)) {
					mPortBuilders.peekLast().mBuilder.withDescription(dHolder.mDetail);
					mPortBuilders.peekLast().mBuilder.withNiceName(dHolder.mShort);
				} else if (mAncestors.get(mAncestors.size()-2).asStartElement().getName()
						.equals(Elements.P_OPTION)) {
					mOptionBuilders.peekLast().mBuilder.withDescription(dHolder.mDetail);
					mOptionBuilders.peekLast().mBuilder.withNiceName(dHolder.mShort);
				}
			} else if (isFirstChild() && event.isStartElement()
					&& (event.asStartElement().getName()
							.equals(Elements.P_INPUT) || event.asStartElement()
							.getName().equals(Elements.P_OUTPUT))) {
	...
	private DocumentationHolder parseDocumentation(final XMLEventReader reader,
			final DocumentationHolder dHolder) throws XMLStreamException {

		Predicate<XMLEvent> pred = Predicates.or(
				EventPredicates.isStartOrStopElement(Elements.XD_SHORT),
				EventPredicates.isStartOrStopElement(Elements.XD_DETAIL),
				EventPredicates.isStartOrStopElement(Elements.P_DOCUMENTATION));
				...
				new EventProcessor() {
					...
					public void process(XMLEvent event)
						...
						if (event.isStartElement()
							...
							reader.next();
							dHolder.mDetail = reader.peek().asCharacters()
									.toString();
